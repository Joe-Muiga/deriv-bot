...
class BotEngine:
    ...
    async def _execute(self, symbol: str, sig: SignalResult) -> bool:
        ...
        # SIGNAL DIRECTION (user-directed, Aug 2026 reversal) — the bot now
        # trades exactly what the strategy/indicator layer computes: no
        # direction flip and no SL/TP swap. This replaces the old
        # "UNIVERSAL SIGNAL INVERSION" block, which unconditionally placed
        # every computed LONG as a SHORT (and vice versa) and swapped the
        # indicator's own stop-loss/take-profit price levels along with it.
        # That behaviour is now opt-in only, gated on
        # config.INVERT_ALL_SIGNALS (default False) — flip that flag back
        # to True to restore the old inversion behaviour; the flip/swap
        # logic itself is unchanged, only whether it runs. meta_labeling
        # .predict_take_trade() remains unused from this path, same as
        # before inversion existed — see meta_labeling.py's
        ...
        if getattr(sig, "contract_kind", "RISE_FALL") != "RISE_FALL" or sig.direction not in ("LONG", "SHORT"):
            ...
            logger.info(
                ...
                f"{sig.direction} (config.INVERT_ALL_SIGNALS=True)"
            )
            # SL/TP PRICE levels swap along with direction, in this same
            # place and at the same time as the flip above — not as a
            # second, separate pass elsewhere, which would risk swapping
            # against a stale/re-fetched signal. The indicator's own
            # stop-loss level becomes the bot's take-profit; its
            # take-profit level becomes the bot's stop-loss. Only runs
            # when INVERT_ALL_SIGNALS is explicitly turned back on.
            ...
            # Default path: trade the signal exactly as the strategy layer
            # computed it — direction as predicted, native stop-loss and
            # take-profit price levels used as the indicator set them.
            logger.debug(
                f"INVERT DISABLED: {symbol} | {strategy} | trading "
                f"{sig.direction} as computed (config.INVERT_ALL_SIGNALS=False)")
        ...
        # only reached if DYNAMIC_STOP_LOSS_ENABLED itself is off.
            ...
            # carries native SL/TP PRICE levels (as computed by the
            # indicator, and inverted/swapped above only if
            # config.INVERT_ALL_SIGNALS is on), those take over entirely —
            # a price-distance stop is
        ...
        # picked indicator's own native price levels (swapped only if
        # config.INVERT_ALL_SIGNALS is on) and
