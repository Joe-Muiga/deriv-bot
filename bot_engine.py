...
BALANCE_REFRESH_EVERY = 60     # poll real balance every 60 seconds
...
class BotEngine:
    ...
    async def run(self):
        ...
        # FIX: register balance callback BEFORE connect() so no updates are missed
        ...
        # Set initial balance in risk manager (already set via on_balance callback
        # but force-set here as well to be safe)
        ...
        # FIX: balance refresh loop — polls real balance every 60s as fallback
        balance_task = asyncio.create_task(self.client.balance_refresh_loop(BALANCE_REFRESH_EVERY))
    ...
    def _on_balance(self, balance: float):
        """Called on every balance update — keeps risk manager in sync."""
    ...
    # FIX: _dashboard_loop is a proper class method (was nested inside orphaned
    # on_authorize snippet, making it a local function and not callable via self)
    ...
    async def _on_contract_result(self, cid: str, msg: dict):
        ...
        # Log result with real balance
        outcome = "✅ WIN" if pnl > 0 else "❌ LOSS"
        logger.info(f"{outcome} | {symbol} | direction={direction} | "
    ...
    async def _init_data(self, symbol: str):
        ...
        logger.info(f"{symbol}: ready | htf={htf_b.count} bars | ltf={ltf_b.count} bars")
